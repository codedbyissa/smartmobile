...
async function getVendas([pageParam, searchParam, cliente, data, baseUrl, codvendedor]) {
  const response = await fetch(
    `http://${baseUrl}/C000074?codvendedor=${codvendedor}&s=${searchParam}&c=${cliente}&d=${data}&page=${pageParam}`
  );
   return response.json(); 
}
...
function Conteudo() {
  ...
  const codvendedor = userPermissions.includes('PermissãoGeralVendas')? 'geral' : user.CODIGO;
        ...
        // query vendas
        const vendas = useInfiniteQuery(    
          ["vendas", searchText, selectedCliente.cod, data, baseUrl, codvendedor ], ({ pageParam = 1 }) => getVendas([pageParam, searchText, selectedCliente.cod, data, baseUrl, codvendedor]), 
          { getNextPageParam: (page) => (page.current_page === page.last_page ? null : page.current_page + 1 ) },
          {  staleTime: 5000, cacheTime: 4000 },
        );

        // load vendas
        const loadMoreVendas = () => {  if (vendas.hasNextPage) {  vendas.fetchNextPage();  }  };
    ...
    return (
        <Container style={{ backgroundColor: appInfo.cor_de_fundo }}>     
          ...
          {vendas.isSuccess? <FlashList
        onEndReached={loadMoreVendas}
        estimatedItemSize={200}
        onEndReachedThreshold={0.5}
        ListFooterComponent={vendas.isFetching ? renderSpinner : null}
        data={(vendas.data.pages.map((page) => page.data)).flat()}    
        ...
        renderItem={({ item }) => {
          ...
          <IconsContent>
          <IconContent style={{backgroundColor: appInfo.cor_principal }} onPress={() => navigation.navigate('ReabrirVenda', { cod: item.CODIGO, })}>
