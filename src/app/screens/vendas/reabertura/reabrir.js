...
const Conteudo = (props) => {
  ...
  const vendas = useQuery( 
    ["vendas", props.cod, baseUrl], () => getVendas([props.cod, baseUrl]), {
      staleTime: 5000, cacheTime: 2000,
      onSuccess: ((data) => {
        if (data) {
          setSelectedCliente(data.CODCLIENTE);
          SetInputobs(data.OBS);

          const copyProductsCart = data.vendas.map((p, index) => ({
            id: index + 1,
            acrescimo: p.ACRESCIMO,
            cod: p.CODPRODUTO,
            codbarra_produto: p.COD_BARRAS,
            desconto: p.DESCONTO,
            desconto_maximo: '',
   ...
   const desc_prod = prcntg_total_prod.map((item) => ((item * descontoFinal) / 100).toFixed(2))
    ...
    const acres_prod = prcntg_total_prod.map((item) => ((item * acrescimoFinal) / 100).toFixed(2))
       ...
       // calcular total final apos desconto e acrescimo
         ...
         fetch(`http://${baseUrl}/C000074/${props.cod}`, {
          method: 'PUT',
          body: JSON.stringify({
          ...
          .then((json) => {
            SalvarPreVendaProduto([json.CODIGO, json.DATA, json.CODVENDEDOR])
          } );       
...
function add_desc(){
 ...
 fetch(`http://${baseUrl}/C000074/${props.cod}`, {
  ...
  function SalvarPreVendaProduto([codnota, data, codvend])

  {
     ...
     Alert.alert(
      ...
      'A Pre-Venda de numero ' + codnota + ' foi atualizada com sucesso.',
      [ 
...
return <View style={{ margin: 10 }}>
  <View style={{ width: '90%' }}>
<DataLab style={{color: appInfo.cor_principal }} key={item.CODIGO}> Codigo: <DataReplyLab  style={{color: appInfo.cor_segundaria }}> {item.CODIGO} </DataReplyLab> </DataLab>
...
<FontAwesomeIcon icon={faCircleCheck}  size={ 30 } color={ appInfo.cor_de_fundo }/>  
        ...
        <InputLab style={{color: appInfo.cor_principal, fontSize: 20, textAlign: 'justify' }}>Você não tem permissão para descontar esse valor, o desconto máximo que pode ser dado para essa venda é { Math.min(...descontos_maximos)  }% </InputLab> 
