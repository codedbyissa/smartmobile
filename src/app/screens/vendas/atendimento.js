import { Alert, Modal, View, Button, ActivityIndicator } from 'react-native';
...
const data_cadastro = dia + '.' + mes + '.' + date.getFullYear();
...
// conteudo do atendimento
   ...
   function Conteudo() {
    ...
    const [finishingModal, setFinishingModal] = useState(false); // modal fechar pre-venda
    ...
    useEffect(() => {
      ...
      // caso o query setores tenha sucesso e o total de setores capturados seja igual a 1
      if(setores.isSuccess && (setores.data.pages.map((page) => page.total)).flat()[0] == 1) {

        // esse unico setor é selecionado por padrão
        const get = (setores.data.pages.map((page) => page.data)).flat()[0]

        // e o seu codigo é setado
        setSelectedSetor(get.CODIGO)
        
       } 
       // mas caso não haja serores capturados, nenhum setor é selecionado
       else if(setores.isSuccess && (setores.data.pages.map((page) => page.total)).flat()[0] == 0){ setSelectedSetor(0) }
    ...
    const requirementsCheck = () => {
      ...
      if(tipoddesconto == '' || dprcntg == 0){
        ...
        setErroMensag(false)
        addProducToCart([selectedProd, produto.data.PRODUTO, produto.data.CODBARRA, produto.data.UNIDADE, 1, MaxDescont.data.DESCONTO_PRODUTO, selectedSetor, setor.data.SETOR, quntd, tipoddesconto, desconto, dprcntg, valorTotal ])
     ...
     const desc_prod = prcntg_total_prod.map((item) => ((item * descontoFinal) / 100).toFixed(2))
      ...
      const acres_prod = prcntg_total_prod.map((item) => ((item * acrescimoFinal) / 100).toFixed(2))
          ...
          function add_desc(){
           ...
           fetch(`http://${baseUrl}/C000074`, {
            method: 'POST',
            body: JSON.stringify({
              CODCAIXA: '000099',
              CODVENDEDOR: user.CODIGO,
              DATA: data_cadastro,
            ...
            .then((json) => {
              SalvarPreVendaProduto(json.CODIGO) 
  ...
  function add_desc(){
   ...
   fetch(`http://${baseUrl}/C000074`, {
    ...
    function SalvarPreVendaProduto(codnota)

    {

        fetch(`http://${baseUrl}/C000075`, {
          method: 'POST',
          body: JSON.stringify({
          ...
          Alert.alert(
            "Pre-Venda bem sucedida",
            'A Pre-Venda de numero ' + codnota + ' foi realizada com sucesso.',
            [ 
    ...
    if (check.isSuccess) {
      ...
      const descontos_maximos = productsCart.map((item) => item.desconto_maximo)
      return(
   ...
   {clientes.isSuccess? <FlashList
    ...
    <BackLab style={{ color: appInfo.cor_de_fundo }}> Voltar </BackLab>
      ...
      <LengthLab2 style={{ color: appInfo.cor_de_fundo }}> {(productsCart).length} </LengthLab2>
      </Length2>
      </Icon>
      </Icons>
      ...
      <Length>
      <FontAwesomeIcon icon={faCircle}  size={ 25 } color={ appInfo.cor_segundaria }/>
      <LengthLab style={{color: appInfo.cor_de_fundo }}> {(productsCart).length} </LengthLab>
      </Length>
      </CartLength>
      ...
      <SearchInput
      style={{ width: '60%', backgroundColor: appInfo.cor_de_fundo, color: appInfo.cor_principal,  borderColor: appInfo.cor_principal }}
...
{tipoddesconto == 'reais'? 
    ...
    <ValueLab style={{color: appInfo.cor_principal }}> {produto.data.PRECOVENDA} </ValueLab>  }        
     ...
     <SaveLab style={{ color: appInfo.cor_de_fundo }}> Fechar Pre-Venda </SaveLab>
     </Save>
    ...
    <Controller
      ...
      { tipodacrescimo_final === 'reais' ?
       ...
       <DisplayLabAcrs> {pxr_acrs} </DisplayLabAcrs>
        ...
        <Confirm style={{backgroundColor: appInfo.cor_principal }} onPress={SalvarPreVenda}> 
      ...
      name="inputsenha"
      render={({ field: { onChange, OnBlur, value} }) => (     
      <ContainerInputSenha style={{ backgroundColor: appInfo.cor_de_fundo }}>
    ...
    { erroSenhaMensag == true? <ErrorMensage>desconto não permitido, senha errada</ErrorMensage> : null }
        ...
        { MaxDescont.isSuccess? <InputLab style={{color: appInfo.cor_principal, fontSize: 20, textAlign: 'justify' }}>Você não tem permissão para descontar esse valor, o desconto máximo que pode ser dado para essa venda é { Math.min(...descontos_maximos)}% </InputLab> : null}
